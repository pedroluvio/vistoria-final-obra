  ...
  for (const [id, photos] of Object.entries(state.photos)) {
    if (photos.length > 0) {
      ...
      for (const p of photos) {
        try {
          const res = await gapi.client.drive.files.get({ fileId: p.id, alt: 'media' });
          const img = document.createElement('img'); img.src = `data:image/jpeg;base64,${btoa(res.body)}`; img.className = 'pdf-report-img';
